 ...
 *
 * <h2>One token, two APIs</h2>
 *
 * The {@code cloud-platform} scope covers both halves of this plugin: the GKE management API at
 * {@code container.googleapis.com} <em>and</em> the cluster's own Kubernetes API server, which accepts a Google
 * access token as a bearer token and maps it to an RBAC subject. That is why a single token source serves both
 * clients — and why the cluster still enforces RBAC independently of the IAM role that minted the token.
 *
...
public final class GoogleTokenSource {
    ...
    private CachedToken mint(GoogleCredentials credentials, PluginHttpClient http, long now) {
        ...
        if (!response.isSuccess()) {
            throw new KubernetesException("K8S_AUTHENTICATION_FAILED",
